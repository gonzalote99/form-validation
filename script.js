...
submit.addEventListener('click', (e) => {
 ...
 if( username.value === "") {
   showMessage( usernameMessage, 'provide name', '#FF0000');
 } else {
   showMessage(usernameMessage , 'valid name' , '#4BB543');
 }
 if( email.value === "") {
  showMessage( emailMessage , 'provide email', '#FF0000');
} else {
  showMessage( emailMessage, 'valid email', '#4BB543');
}

if( password1.value === "") {
  showMessage(password1Message , 'provide password', '#FF0000');
} else {
  showMessage(password1Message , 'valid password', '#4BB543');
}

if( password2.value === "") {
  showMessage( password2Message, 'confirm', '#FF0000');
} else {
